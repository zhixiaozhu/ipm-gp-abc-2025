...
# input_data is a full dataset.
# sizeNext_group: used to label size group

def sizeNext_group(input_data):
    d = input_data.copy()
    d['label_sizeNext'] = np.floor(d['sizeNext'])
    d.loc[d['label_sizeNext'] < 0, 'label_sizeNext'] = 0
    d.loc[d['label_sizeNext'] > 4, 'label_sizeNext'] = 4
...
def ageNext_group(input_data):
    ...
    # ageNext [2, 5] young(1); [6, 8] middle(2); >10 white beard
    d.loc[d['ageNext'] > 5, 'label_ageNext'] = 2
    d.loc[d['ageNext'] >= 8, 'label_ageNext'] = 3
    
    return d

def zero_fixed(result, bysize, byage):
...
def num_indi_group(input_data, bysizeNext, byageNext):
...
def prob_indi_group(input_data, bysizeNext, byageNext):
...
# individuals in size groups, by their states at current time.


def size_group(input_data):
    d = input_data.copy()
    d['label_size'] = np.floor(input_data['size'])
    d.loc[d['label_size'] < 0, 'label_size'] = 0
    d.loc[d['label_size'] > 4, 'label_size'] = 4
...
def age_group(input_data):
    ...
    d.loc[d['age'] >= 8, 'label_age'] = 3
    
    return d

# calcualte the total/mean number of breeders/flowering stlks/survivors in each group.

def num_ffs_group(input_data, target, bysize, byage, mean=True):
...
def fer_dis(input_data, target, bysizeNext, byageNext):
