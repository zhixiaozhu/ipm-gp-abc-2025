...
whole_year.true_z_allsteps = True
whole_year.abc_index = np.load(open(file = os.getcwd()+f"/ABC_details/p_index_10.pkl", mode="rb"))
whole_year.weight = np.load(open(file = os.getcwd()+f"/ABC_details/weight10.pkl", mode="rb"))

b = np.array(whole_year.prediction_para(total_samples=5000, abc=True),  dtype=object)
np.save(open(file = os.getcwd()+"/true_popu/simulations/simu_abc_z.pkl", mode="wb"), b)
