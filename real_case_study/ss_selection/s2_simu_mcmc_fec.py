...
if __name__ == '__main__':
    # We'll refit the m_fec model only, and keep the other vital rate models as the MLE.
    # 1 #####################################################################################################
    # Set-up
    target = 'm_fec'
    ...
    IPM_pret_fec = Perted_IPM(popu_data=popu_dataset, GPmodel_true=models_true, target=target, NB=False)
    lower_p = 0
    upper_p = 5
    m_size_p = 100
    max_age_p = 13
    ...
    kernel = gpflow.kernels.RBF(lengthscales=np.array([50,50])) 
    m_fec_new = gpflow.models.GPMC(data=(IPM_pret_fec.X_fec, IPM_pret_fec.Y_fec), kernel=kernel, likelihood=l)
    ...
    if simu_mcmc_mode == 'ON':
        ...
        hmc = tfp.mcmc.HamiltonianMonteCarlo(
            target_log_prob_fn=hmc_helper.target_log_prob_fn, num_leapfrog_steps=10, step_size=f64(0.02)
        )
        ...
        pickle.dump(samples, open(file = os.getcwd()+"/mcmc_samples/m_fec" +"/samples.pkl", mode="wb"))
        pickle.dump(parameter_samples, open(file = os.getcwd()+"/mcmc_samples/m_fec" +"/parameter_samples.pkl", mode="wb"))
        
        #  assign samples to our models
        IPM_pret_fec.mcmc_para_sample = parameter_samples
    
    elif simu_mcmc_mode == 'OFF':
        print('\n\n\n' + 'Loading MCMC samples' + '\n\n\n')
        samples = pickle.load(open(file = os.getcwd()+"/mcmc_samples/m_fec" +"/samples.pkl", mode="rb"))
        parameter_samples = pickle.load(open(file = os.getcwd()+"/mcmc_samples/m_fec" +"/parameter_samples.pkl", mode="rb"))
    ...
    # 4 #####################################################################################################
    # ploting
    param_to_name = {param: name for name, param in gpflow.utilities.parameter_dict(m_fec_new).items()}
    if plot_mode == 'ON':
        print('\n\n\n' + 'Plotting' + '\n\n\n')

        name_to_index = {param_to_name[param]: i for i, param in enumerate(m_fec_new.trainable_parameters)}
        hyperparameters = [
            ".kernel.lengthscales",
            ".kernel.variance"
        ]
        ...
        marginal_samples(samples, m_fec_new.trainable_parameters, "unconstrained variable samples", param_to_name)
    ...
    if popu_all_mode == 'ON':
        ...
        summary_data = IPM_pret_fec.obsVSsimu_fun_parallel(rep=1, random_seed=1, 
                                                             MCMC_boolean_list='All')      

        pickle.dump(summary_data, open(file = os.getcwd() + "/mcmc_samples/m_fec" + "/summary_data.pkl", mode="wb"))
        IPM_pret_fec.nlog_post = np.array(summary_data['nlpo'])
        IPM_pret_fec.nlog_likeli = np.array(summary_data['nll'])

    elif popu_all_mode == 'OFF':
        print('\n\n\n' + 'Loading summary_data for all the MCMC samples' + '\n\n\n')
        summary_data = pickle.load(open(file = os.getcwd() + "/mcmc_samples/m_fec" + "/summary_data.pkl", mode="rb"))
        IPM_pret_fec.nlog_post = np.array(summary_data['nlpo'])
        IPM_pret_fec.nlog_likeli = np.array(summary_data['nll'])
    else:
    ...
    if popu_opt_mode == 'ON':
        ...
        pickle.dump(summary_opt, 
                    open(file = os.getcwd() + "/mcmc_samples/m_fec" + "/summary_opt.pkl", mode="wb"))
        ...
        pickle.dump(summary_around_opt, 
                        open(file = os.getcwd() + "/mcmc_samples/m_fec/summary_around_opt.pkl", mode="wb"))

    elif popu_opt_mode == 'OFF':
        print('\n\n\n' + 'Loading summary_data for the opt MCMC samples' + '\n\n\n')
        summary_opt = pickle.load(open(file = os.getcwd() + "/mcmc_samples/m_fec" + "/summary_opt.pkl", mode="rb"))
        summary_around_opt = pickle.load(open(file = os.getcwd() + "/mcmc_samples/m_fec" + "/summary_around_opt.pkl", mode="rb"))
