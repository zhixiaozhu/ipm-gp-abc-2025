...
if __name__ == '__main__':
    # We'll refit the m_sur model only, and keep the other vital rate models as the MLE one.
    # 1 #####################################################################################################
    # Set-up
    target = 'm_sur'
    ...
    if simu_mcmc_mode == 'ON':
        ...
        hmc = tfp.mcmc.HamiltonianMonteCarlo(
            target_log_prob_fn=hmc_helper.target_log_prob_fn, num_leapfrog_steps=10, step_size=0.05
        )
        ...
        pickle.dump(samples, open(file = os.getcwd()+"/mcmc_samples/m_sur" +"/samples.pkl", mode="wb"))
        pickle.dump(parameter_samples, open(file = os.getcwd()+"/mcmc_samples/m_sur" +"/parameter_samples.pkl", mode="wb"))
    ...
    # 4 #####################################################################################################
    # ploting
    param_to_name = {param: name for name, param in gpflow.utilities.parameter_dict(m_sur_new).items()}
    if plot_mode == 'ON':
        print('\n\n\n' + 'Plotting' + '\n\n\n')
        ...
        param_to_name = {param: name for name, param in gpflow.utilities.parameter_dict(m_sur_new).items()}
        name_to_index = {param_to_name[param]: i for i, param in enumerate(m_sur_new.trainable_parameters)}
        hyperparameters = [
            ".kernel.lengthscales",
            ".kernel.variance"
        ]
        ...
        marginal_samples(samples, m_sur_new.trainable_parameters, "unconstrained variable samples", param_to_name)
    ...
    if popu_all_mode == 'ON':
        ...
        summary_data = IPM_pret_sur.obsVSsimu_fun_parallel(rep=1, random_seed=1, 
                                                             MCMC_boolean_list='All')      

        pickle.dump(summary_data, open(file = os.getcwd() + "/mcmc_samples/m_sur" + "/summary_data.pkl", mode="wb"))
    ...
    if popu_opt_mode == 'ON':
        ...
        summary_opt = IPM_pret_sur.simuVSsimu_singleModel_fun_parallel(rep=rep, random_seed=1,
                                            interested_models='Opt', evaluate_at_training=False)

        pickle.dump(summary_opt, 
                    open(file = os.getcwd() + "/mcmc_samples/m_sur" + "/summary_opt.pkl", mode="wb"))
        
        summary_around_opt = IPM_pret_sur.simuVSsimu_fun_parallel(rep=rep, random_seed=1, 
        ...
        pickle.dump(summary_around_opt, 
                        open(file = os.getcwd() + "/mcmc_samples/m_sur/summary_around_opt.pkl", mode="wb"))
