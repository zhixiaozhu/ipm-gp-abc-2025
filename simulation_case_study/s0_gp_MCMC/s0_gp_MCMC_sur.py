...
num_burnin_steps = ci_niter(10000)
num_samples = ci_niter(5000)

hmc = tfp.mcmc.HamiltonianMonteCarlo(
    target_log_prob_fn=hmc_helper.target_log_prob_fn, num_leapfrog_steps=10, step_size=f64(0.0001)
)
adaptive_hmc = tfp.mcmc.SimpleStepSizeAdaptation(
    hmc, num_adaptation_steps=int(num_burnin_steps * 0.7), target_accept_prob=f64(0.8), adaptation_rate=f64(0.1)
)
...
pickle.dump(samples, open(file = os.getcwd()+f"/true_popu/glm_mle_true_population/MCMC/gp/sep/samples_sur.pkl", mode="wb"))